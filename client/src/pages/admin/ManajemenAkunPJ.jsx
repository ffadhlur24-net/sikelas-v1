  ...
  return (
    <div className="animate-fade-in">
      <div className="page-header">
        <h1 className="page-title">Menejemenn Akun PJ</h1>
        <p className="page-subtitle">Kelola persetujuan akun PJ baru dan status keaktifan mahasiswa.</p>
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border-color)' }}>
                <th style={{ padding: '12px 16px', color: 'var(--text-secondary)', fontWeight: '500' }}>Nama PJ</th>
                <th style={{ padding: '12px 16px', color: 'var(--text-secondary)', fontWeight: '500' }}>Kontak</th>
                <th style={{ padding: '12px 16px', color: 'var(--text-secondary)', fontWeight: '500' }}>Prodi</th>
            ...
            <tbody>
              {users.length > 0 ? users.map((u) => ( // <-- Gunakan kurung biasa (
                <tr key={u.id} style={{ borderBottom: '1px solid var(--border-color)' }}>

                  {/* Kolom 1 */}
                  <td style={{ padding: '16px' }}>
                    <div style={{ fontWeight: '500' }}>{u.nama}</div>
                    <div className="text-sm text-muted">{u.nim}</div>
                  </td>
                  ...
                  {/* Kolom 3 */}
                  <td style={{ padding: '16px' }}>{u.prodi || '-'}</td>

                  {/* Kolom 4 */}
                  <td style={{ padding: '16px' }}>
                    {u.status === 'aktif' && <span className="badge badge-success">Aktif</span>}
                    {u.status === 'pending' && <span className="badge badge-warning">Menunggu</span>}
