...
function VerifyEmail() {
    ...
    const handleVerify = async (e) => {
        ...
        try {
            ...
            setError(err.response?.data?.error || 'Verifikasi gagal. Pastikan kode OTP benar.')
        } finally {
            setLoading(false)
        }
    }
    ...
    const handleResend = async () => {
        ...
        try {
            ...
            setError(err.response?.data?.error || 'Gagal mengirim OTP. Pastikan email kampus Anda aktif.')
        } finally {
    ...
    return (
        <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f8fafc', padding: '16px' }}>
            <div className="card-flat" style={{ width: '100%', maxWidth: '420px', background: '#fff', borderRadius: '16px', padding: '32px', textAlign: 'center', boxShadow: '0 10px 25px rgba(0,0,0,0.05)' }}>
                <div style={{ fontSize: '48px', marginBottom: '12px' }}>📩</div>
                <h2 style={{ fontSize: '20px', fontWeight: 'bold', color: '#0f172a', marginBottom: '8px' }}>Verifikasi Email Kampus</h2>
                <p style={{ fontSize: '13px', color: '#64748b', marginBottom: '24px', lineHeight: '1.5' }}>
                    Kode OTP 6-digit telah dikirimkan ke inbox email: <br />
